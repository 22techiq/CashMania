...
async function downloadPDF(){
    ...
    doc.setFontSize(10);
    doc.text(`Generated By: ${result.generatedBy}`,15,38);
    doc.text(`Generated On: ${result.generatedOn}`,15,44);
    doc.text(`Report: ${result.filter}`,15,50);

    doc.setFontSize(13);
    doc.text(`Total Collections: KSh ${Number(result.total).toLocaleString()}`,15,62);
    doc.text(`Transactions: ${result.count}`,135,62);

    /* ---------- TABLE ---------- */

    const rows = result.collections.map(item => [
        item.date,
        item.receipt,
        item.service,
        item.payment,
    ...
    doc.autoTable({
        startY:72,
        head:[["Date","Receipt","Service","Payment","Cashier","Amount"]],
        body:rows,
        theme:"grid",
        headStyles:{
            fillColor:[8,127,115]
    ...
    doc.save(
      `MASC_Report_${new Date().toISOString().slice(0,10)}.pdf`
    );
